...
def data_generator(corpus, neg_num, window):
    ...
    for sentence in corpus:
        ...
        for i in range(window, len(sentence) - window):
            ...
            y.append(surrounding + get_negative_sample(x, nb_word, nb_negative))
    x, y = np.array(x), np.array(y)
    z = np.zeros(len(x), len(y))  # z = np.zeros((len(x), nb_negative+len(surrounding)))
    z[:, :len(surrounding)] = 1
...
def build_model():
    ...
    biases = Embedding(nb_word, name='b')(samples)

    input_vec_dot = Lambda(lambda x: K.batch_dot(x[0], K.expand_dims(x[1], 2)))([weights, input_vec])

    # !!!
    add_biases = Lambda(lambda x: K.reshape(x[0] + x[1], shape=(-1, len_samples)))([input_vec_dot, biases])
...
def most_similar(embeddings, word):
